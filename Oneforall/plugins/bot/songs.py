...
from config import BANNED_USERS, SONG_DOWNLOAD_DURATION_LIMIT, SONG_DOWNLOAD_DURATION
...
from Oneforall.utils.inline.rich import caption_blocks, deliver_rich, edit_rich
...
async def _panel_blocks(_, token, idx, notice=None):
    ...
    caption = (
        f"<b>Title:</b> <a href=\"{item['link']}\">{html.escape(item['title'])}</a>\n"
        f"<b>Channel:</b> {html.escape(item['channel'])}\n"
        f"<b>Duration:</b> {item['duration'] or 'Live'}\n"
        f"<b>Views:</b> {item['views']}\n"
        f"<b>Published:</b> {item['published']}"
    )
    ...
    blocks.append(
        _row(
            _button("🎵 Audio", f"SONG a|{token}|{idx}", ButtonStyle.SUCCESS),
            _button("🎬 Video", f"SONG v|{token}|{idx}", ButtonStyle.PRIMARY),
        )
    )
    total = len(results)
    if total > 1:
    ...
    blocks.append(
        _row(
            _button("Watch on YouTube", style=ButtonStyle.DEFAULT, url=item["link"]),
...
async def _prepare_audio(path, status, _):
    ...
    await status.show("Converting Audio...", force=True)
    out = _temp("mp3")
...
async def _prepare_video(path, status, _):
    ...
    await status.show("Processing Video...", force=True)
    out = _temp("mp4")
    async with _convert_gate:
        try:
            code, _out = await _run(
        ...
        [
            [
                InlineKeyboardButton(
                    text="Watch on YouTube", url=item["link"], style=ButtonStyle.PRIMARY
                ),
                InlineKeyboardButton(
                    text=_["CLOSE_BUTTON"],
...
async def _send_file(client, cq, _, item, kind, path, status):
    ...
    try:
        ...
        caption = (
            ...
            f"<b>Channel:</b> {html.escape(item['channel'])}\n"
            f"<b>Duration:</b> {item['duration'] or 'Live'}\n"
            f"<b>Requested By:</b> {cq.from_user.mention}\n"
        ...
        async def progress(current, total):
            ...
            await status.show(f"Uploading... {percent}%")

        await status.show("Uploading... 0%", force=True)
...
async def _download_flow(client, cq, _, token, idx, kind):
    ...
    path = None
    try:
        word = "Audio" if kind == "a" else "Video"
        await status.show(f"Downloading {word}...", force=True)
        path = await _fetch(item, kind)
        _in_use[path] = _in_use.get(path, 0) + 1
        await _send_file(client, cq, _, item, kind, path, status)
    except SongError as e:
        await _restore_panel(_, panel, token, idx, "Download failed.")
        return
    except Exception:
        await _restore_panel(_, panel, token, idx, "Download failed.")
        return
...
async def song_command(client, message, _):
    ...
    if now - _cooldown.get(user.id, 0) < COOLDOWN:
        notice = await message.reply_text("Please wait before requesting again.")
    ...
    if not query:
        return await message.reply_text(
            _["song_2"], reply_markup=_close_markup(_, user.id)
        )
    ...
    try:
        ...
        return await mystic.edit_text(
            _["play_3"], reply_markup=_close_markup(_, user.id)
        )
    if not results:
        return await mystic.edit_text(
            _["song_5"], reply_markup=_close_markup(_, user.id)
        )
...
@app.on_callback_query(filters.regex(r"^SONG ") & ~BANNED_USERS)
@languageCB
async def song_callback(client, cq, _):
    try:
        ...
        try:
            await cq.answer("Action failed.", show_alert=True)
        except Exception:
            pass
...
async def _handle(client, cq, _):
    ...
    if action == "d":
        if str(user.id) != rest and user.id not in SUDOERS:
            return await cq.answer("Not your button.", show_alert=True)
        await cq.answer()
    ...
    if key in _busy:
        return await cq.answer("Download already in progress...", show_alert=True)
    if not item["seconds"]:
        return await cq.answer(_["song_3"], show_alert=True)
    if item["seconds"] > SONG_DOWNLOAD_DURATION_LIMIT:
        return await cq.answer(
            _["play_4"].format(SONG_DOWNLOAD_DURATION, item["duration"]), show_alert=True
        )
    _busy.add(key)
    try:
        await cq.answer()
